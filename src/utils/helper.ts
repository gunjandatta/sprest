...
class _Helper {
    ...
    static copyFileToHostWeb(fileUrl:string, dstFolder:(string | IFolder), overwriteFl?:boolean, rootWebFl?:boolean) {
        ...
        let web = (<IWeb><any>new Web(rootWebFl ? ContextInfo.siteServerRelativeUrl : null));
        ...
        if(typeof(dstFolder) === "string") {
            ...
            (<IWeb><any>new Web())
                ...
                .execute((content:IBase) => {
                    ...
                    (<IFolder>dstFolder).Files().add(true, fileName, content.response)
                        // Execute the request
                        .execute((file:IFile) => {
    ...
    // Method to copy a file in this app web to the host web
    static copyFilesToHostWeb(fileUrls:Array<string>, folderUrls:Array<string>, overwriteFl?:boolean, rootWebFl?:boolean, idx?:number, promise?:Promise, files?:Array<IFile>, folders?:Array<IFolder>) {
        files = files ? files : [];
    ...
    static createSubFolders(folder:IFolder, subFolderUrl:string, promise?:Promise): Promise {
    ...
    static getFolder(web:IWeb, folderUrl:string, createFl?:boolean) {
        let dstFolder = null;
        let promise = new Promise();
    ...
    static removeEmptyFolders(web:IWeb, folderUrls:Array<string>) {
        let promise = new Promise();
    ...
    static removeFile(web:IWeb, fileUrl:string) {
        let promise = new Promise();
        let folder = null;
        ...
        // Get the file
        folder.Files(folders[folders.length-1]).execute((file:IFile) => {
            // See if it exists
            if(file.Exists) {
    ...
    static removeFiles(web:IWeb, fileUrls:Array<string>, idx?:number, promise?:Promise) {
        idx = idx ? idx : 0;
        promise = promise ? promise : new Promise();
