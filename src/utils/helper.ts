...
import { Base, MethodInfo, RequestType } from ".";
...
export const Helper: IBaseHelper = {
    ...
    addBaseReferences: (base: IBase, obj: any) => {
        ...
        obj["addMethods"] = Helper.addMethods;
        obj["base"] = base.base;
        obj["done"] = base.done;
        ...
        obj["updateMetadataUri"] = this.updateMetadataUri;
            ...
            if (value && value.__deferred && value.__deferred.uri) {
                ...
                if (base[key] && base[key].results) {
                    ...
                    if (base[key].results.length == 0 || typeof (base[key].results[0]) === "object") {
                        ...
                        // Add the methods
                        Helper.addMethods(objCollection, objCollection);
    ...
    executeMethod: (base: IBase, methodName: string, methodConfig: IMethodInfo, args?: any) => {
        ...
        // Set the properties
        obj.base = base.base ? base.base : base as any;
        obj.getAllItemsFl = methodInfo.getAllItemsFl;
        obj.parent = base as any;
            ...
            if (idxEnd > idxStart && idxStart > -1) {
                ...
                if (xml[idxEnd - 1] == "/") {
                    ...
                    if (value.indexOf("<d:") == 0 && idxEnd > idxStart) {
                        // Set the value
                        objData[propName] = Helper.parseXML(value);
                    } else {
                        // Set the value
        ...
        if (results) {
            ...
            if (obj["results"].length > 0) {
                ...
                for (let result of results) {
                    // Add the base references
                    Helper.addBaseReferences(obj, result);

                    // Update the metadata
                    Helper.updateMetadata(obj, result);

                    // Add the methods
                    Helper.addMethods(result, result);
                }
            }
        }
    },
